...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    char buf[BUF_SIZE + 1];
    int read_num = read(client_fd, &buf, BUF_SIZE);
    if (read_num == -1) {
        perror("server: read");
        exit(1);
    }
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    char buf2[2 * BUF_SIZE + 1];
    strncpy(buf2, usernames[client_index].username, strlen(usernames[client_index].username));
    strcat(buf2, ": ");
    strcat(buf2, buf);

    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (usernames[i].sock_fd != -1) {
            if (write(usernames[i].sock_fd, buf2, strlen(buf2)) != strlen(buf2)) {
                usernames[client_index].sock_fd = -1;
                return fd;
            }
        }
