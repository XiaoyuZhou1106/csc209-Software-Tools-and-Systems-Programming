...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
	struct pixel **image_pixel = malloc(sizeof(struct pixel*) * height);
	
	for (int i = 0; i < height; i++) {
		image_pixel[i] = malloc(sizeof(struct pixel) * width);
	}
	
	fseek(image, pixel_array_offset, SEEK_SET);
	for (int j = 0; j < height; j++) {
		for (int k = 0; k < width; k++) {
			fread(&image_pixel[j][k].blue, 1, 1, image);
			fread(&image_pixel[j][k].green, 1, 1, image);
			fread(&image_pixel[j][k].red, 1, 1, image);
		}
	}
	return image_pixel;
}
